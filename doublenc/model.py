...
class ResUNet34_2task(torch.nn.Module):
    ...
    def __init__(self, in_ch, simclr, bilinear=False, layer_return=False, bias=False):
        ...
        #self.firstconv = simclr.module.backbone.firstconv
        #self.enc1 = simclr.module.backbone.enc1
        #self.enc2 = simclr.module.backbone.enc2
        #self.enc3 = simclr.module.backbone.enc3
        ...
        self.firstconv_task1 = torch.nn.Sequential(*[torch.nn.Conv2d(in_ch, self.base_channel, 3, padding=1),
                                               torch.nn.BatchNorm2d(self.base_channel),
                                               torch.nn.ReLU(inplace=True)])
        self.enc1_task1 = self._make_layer(block, self.filters[0], self.filters[0], 3, 1)
        self.enc2_task1 = self._make_layer(block, self.filters[0], self.filters[1], 4, 2)
        self.enc3_task1 = self._make_layer(block, self.filters[1], self.filters[2], 6, 2)
        #self.firstconv_task1 = simclr.module.backbone.firstconv
        #self.enc1_task1 = simclr.module.backbone.enc1
        #self.enc2_task1 = simclr.module.backbone.enc2
        #self.enc3_task1 = simclr.module.backbone.enc3
        self.enc4_task1 = self._make_layer(block, self.filters[2], self.filters[3], 3, 2)

        self.centerblock_task1 = Double_conv(self.filters[3], self.filters[3], bias=bias)
        ...
        self.comb1 = self.decoder(self.filters[0], self.filters[0], 0., bias=bias)
        self.comb2 = self.decoder(self.filters[1], self.filters[1], 0., bias=bias)
        self.enc4_conv = torch.nn.Conv2d(self.filters[3], 1, kernel_size=1)
        self.enc3_conv = torch.nn.Conv2d(self.filters[2], 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # enc_task1
        out_first_task1 = self.firstconv_task1(x)  # [4, 16, 512, 512]
        out_enc1_task1 = self.enc1_task1(out_first_task1)# [4, 16, 512, 512
        out_enc2_task1 = self.enc2_task1(out_enc1_task1) # [4, 32, 256, 256]
        out_enc3_task1 = self.enc3_task1(out_enc2_task1) # [4, 64, 128, 128]
        
        out_enc4_task1 = self.enc4_task1(out_enc3_task1) # [4, 128, 64, 64]
        out_center_task1 = self.centerblock_task1(out_enc4_task1)

        # conbination
        vessel4 = self.enc4_conv(out_enc4_task1).sigmoid()
        vessel3 = self.enc3_conv(out_enc3_task1).sigmoid()
        vessel2 = self.enc2_conv(out_enc2_task1).sigmoid()
        vessel1 = self.enc1_conv(out_enc1_task1).sigmoid()

        # enc_task2
       
        out_first = self.firstconv(x)  # [4, 16, 512, 512]
        out_enc1 = self.enc1(out_first)
        #out_enc1 = self.comb1(out_enc1 + out_enc1_task1)
        out_enc2 = self.enc2(out_enc1) # [4, 32, 256, 256]
        #out_enc2 = self.comb2(out_enc2 + out_enc2_task1)
        out_enc3 = self.enc3(out_enc2) # [4, 64, 128, 128]
        out_enc3 = self.e_nl3(out_enc3,vessel3)
        out_enc4 = self.enc4(out_enc3) # [4, 128, 64, 64]
        out_enc4 = self.e_nl4(out_enc4,vessel4)
        out_center = self.centerblock(out_enc4) # [4, 128, 64, 64
    
        # dec_task1
        out_dec4_task1 = self.dec4_task1(torch.cat([out_enc4_task1, out_center_task1], dim=1)) #
        out_up3_task1 = self.up3_task1(out_dec4_task1) 
        out_dec3_task1 = self.dec3_task1(torch.cat([out_enc3_task1, out_up3_task1], dim=1))
        ...
        out_dec1_task1 = self.dec1_task1(torch.cat([out_enc1_task1, out_up1_task1], dim=1))  #[4, 16, 512, 512]
