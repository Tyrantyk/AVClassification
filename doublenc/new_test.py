...
def av_cla_eval(pre, label, ves, old):
    ...
    matrix_old = confusion_matrix(label, old)

    tp, fn, fp, tn = matrix[1, 1], matrix[1, 2], matrix[2, 1], matrix[2, 2]

    new_eval = (tp + tn) / (tp + tn + fp + fn + matrix[1, 0] + matrix[2, 0])
    tp, fn, fp, tn = matrix_old[1, 1], matrix_old[1, 2], matrix_old[2, 1], matrix_old[2, 2]
...
def test_in_train_AVDRIVE(test_loader, net, best_yi_all):
    ...
    with torch.no_grad():
        ...
        for i, (test_img, test_label, test_ves) in enumerate(test_loader):
            ...
            old_test_pre = torch.nn.functional.softmax(test_yfake_ves, dim=1).clone()
            old_test_pre = torch.argmax(old_test_pre, dim=1)

            label = label.detach().cpu().numpy()
            test_yfake_ves = test_yfake_ves.detach().cpu().numpy()
            old_test_pre = old_test_pre.detach().cpu().numpy()            

            ves_acc, wrong_rate, ves_pre_gt = ves_seg_eval(ves_seg, test_ves_pad, 0.1)
            old_eval, new_eval, pre_dif_show = av_cla_eval(test_yfake_ves, label, ves_pre_gt, old_test_pre)
